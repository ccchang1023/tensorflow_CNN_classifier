...
train_images = np.load("cat_dog_flower_data.npy")
train_labels = np.load("cat_dog_flower_label.npy")
...
NUM_OF_TESTING = len(test_image_paths)
test_images = np.array(test_images,dtype=np.float32)
test_images = test_images.reshape(NUM_OF_TESTING,IMAGE_PIXELS)

## Define labels of testing data ##
test_labels = np.zeros(shape=(NUM_OF_TESTING,3))
print ("testing num: %d")%(NUM_OF_TESTING);
for i in range(NUM_OF_TESTING):
    if i < 100:
        test_labels[i] = [1,0,0] #cats
    elif i < 200:
        test_labels[i] = [0,1,0] #dogs
    else:
        test_labels[i] = [0,0,1] #flowers
test_predict = tf.argmax(y_conv,1)
test_ans = tf.argmax(ys,1)
test_result = test_predict.eval(feed_dict={xs:test_images,ys:test_labels,keep_prob:1.0})
